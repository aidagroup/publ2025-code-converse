...
# from scipy.stats import norm, poisson  # For Gaussian fit
...
# def create_kde_plots(final_rewards, kernels=['gaussian'], output_path_base="kde_plot"):
#     """Generates filled KDE plots with specified kernels using KernelDensity."""

#     if not final_rewards:
#         print("No data to plot.")
#         return

#     sns.set_theme(style="whitegrid")  # Apply a visually appealing Seaborn theme
#     plt.figure(figsize=(12, 8))  # Increase figure size for better readability

#     colors = {"Adam": "#e41a1c", "RAdam": "#377eb8"}  # Distinct and accessible colors

#     for optimizer, rewards in final_rewards.items():
#         rewards = np.array(rewards)
#         mu, std = norm.fit(rewards)  # Fit a normal distribution to get mean and std

#         rewards_reshaped = rewards.reshape(-1, 1)
#         for kernel in kernels:
#             kde = KernelDensity(kernel=kernel, bandwidth=80).fit(rewards_reshaped)  # Adjust bandwidth
#             x_vals = np.linspace(rewards.min() - 500, rewards.max()+500, 1000).reshape(-1,
#                                                                                           1)  # Dynamic x-axis range
#             y_vals = np.exp(kde.score_samples(x_vals))

#             color = colors[optimizer]
#             plt.plot(x_vals, y_vals, label=f"{optimizer} (KDE)", linewidth=2, color=color)
#             plt.fill_between(x_vals.flatten(), 0, y_vals, alpha=0.4, color=color)

#             # Overlay Gaussian distribution
#             gauss_y = norm.pdf(x_vals.flatten(), mu, std)
#             plt.plot(x_vals, gauss_y, label=f"{optimizer} (Gaussian Fit)", linestyle='--', color=color, alpha=0.7)

#     plt.xlabel("Final Reward Value", fontsize=14)
#     plt.ylabel("Density", fontsize=14)
#     plt.title("Distribution of Final Reward Values by Optimizer", fontsize=16, fontweight='bold')
#     plt.legend(fontsize=12)
#     plt.grid(axis='y', linestyle='--', alpha=0.7)
#     plt.xlim(x_vals.min(), x_vals.max())  # Set x-axis limits

#     # Add a vertical line at zero reward for reference
#     plt.axvline(0, color='black', linestyle='-', linewidth=0.8, label='Zero Reward')

#     sns.despine()  # Remove top and right spines for a cleaner look
#     plt.tight_layout()
#     plt.savefig(f"{output_path_base}_Adam_RAdam_Gaussian_Overlay_All.png", dpi=300)  # High-resolution save
#     plt.show()
